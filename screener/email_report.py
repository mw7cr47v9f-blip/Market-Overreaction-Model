...
def build_email(data_dir):
    ...
    H = [f"<h2 style='{ff};color:#0f766e;margin:0 0 6px'>Cragent.ai daily &middot; {esc(scan_date)}</h2>"]
    if confirmed:
        H.append(f"<h3 style='{ff};color:#0f766e;margin:12px 0 4px'>Buy now &mdash; confirmed entry today</h3><ul style='{ff};font-size:14px'>")
        for c in confirmed:
            H.append(f"<li><b>{esc(c.get('ticker'))}</b> ({esc(c.get('sector',''))}) &mdash; confirmed breakout @ {esc(c.get('entry_price'))}, dropped {esc(c.get('drop_date'))}</li>")
        H.append("</ul>")
    if sells or hold_due:
    ...
    if pending:
        ...
        for p in pending:
            H.append(f"<li>{esc(p.get('ticker'))} ({esc(p.get('sector',''))}) — day {esc(p.get('day','?'))}/15 since drop {esc(p.get('drop_date'))}</li>")
        H.append("</ul>")
    if tally.get("n_closed"):
